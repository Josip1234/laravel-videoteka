@section('create')
 <div class="flex items-center">
      <h2 class="text-2xl font-bold text-white mb-6 text-center">Unos nove posudbe</h2>
    </div>
     <div class="flex items-center justify-center">
        <form action="{{ route('posudba.spremi',$clanska_iskaznica) }}" method="post" class="space-y-5">
            @csrf
            <label for="broj_iskaznice" class="block text-gray-300 mb-2 text-sm font-medium">Broj iskaznice</label>
                ...
                <input type="number" name="zakasnina" id="zakasnina" min="0.00" step="0.11" value="{{ old('zakasnina')}}" class="w-full px-4 py-2 bg-gray-700 text-white rounded-lg border border-gray-600 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition duration-200">
                    @error('zakasnina')
                    <p class="text-red-600 text-sm">{{ $message }}</p>
                @enderror
                  ...
                  <label for="brojPopisa" class="block text-gray-300 mb-2 text-sm font-medium">Odaberi popis</label>
                    <select name="brojPopisa" id="brojPopisa" class="w-full px-4 py-2 bg-gray-700 text-white rounded-lg border border-gray-600 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition duration-200">
                        @foreach ($posudbe as $posudba)
                            <option value="{{ $posudba->brojPopisa }}" @selected(old('brojPopisa'))>{{ $posudba->brojPopisa }}</option>
                        @endforeach
                    </select>
                    @error('brojPopisa')
                    <p class="text-red-600 text-sm">{{ $message }}</p>
