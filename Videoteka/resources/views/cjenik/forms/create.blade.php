@section('create')
 <div class="flex items-center">
      <h2 class="text-2xl font-bold text-white mb-6 text-center">Unos novog cijenika</h2>
    </div>
     <div class="flex items-center justify-center">
<form action="{{ route('cjenik.spremi') }}" method="post" class="space-y-5">
    @csrf
  ...
  <label for="id_filma" class="block text-gray-300 mb-2 text-sm font-medium">Odaberi film:</label>
    <select name="id_filma" id="id_filma" class="w-full px-4 py-2 bg-gray-700 text-white rounded-lg border border-gray-600 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition duration-200">
        @foreach ($filmovi as $film)
            <option value="{{ $film->id_filma }}" @selected(old('id_filma',$film->id_filma))>{{ $film->naziv }}</option>
        @endforeach
    </select>
    @error('id_filma')
    ...
    <input type="number" name="cijena_filma" id="cijena_filma" min="0.01" step="0.10" value="{{ old('cijena_filma') }}" class="w-full px-4 py-2 bg-gray-700 text-white rounded-lg border border-gray-600 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition duration-200">
    @error('cijena_filma')
        <p class="text-red-600 text-sm">{{ $message }}</p>
    @enderror
        ...
        @foreach ($vrsta as $vrsta)
            <option value="{{ $vrsta->id_vrste_cjenika }}" @selected(old('id_vrste_cjenika',$vrsta->id_vrste_cjenika))>{{ $vrsta->naziv }}</option>
        @endforeach
    </select>
